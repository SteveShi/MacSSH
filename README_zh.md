...
3. **`MacSSHTerminal` (终端渲染模块)**：桥接 Ghostty 终端引擎（`MactermKit` / `MactermKitCore.xcframework`）与 Metal 硬件加速渲染视图。
...
| 层级 | 技术 |
|------|-----|
| 终端引擎 | [Ghostty](https://ghostty.org/) (`MactermKit` / `MactermKitCore.xcframework`) |
